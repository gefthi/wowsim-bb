...
func NewSimulator(cfg *config.Config, simCfg SimulationConfig, seed int64) *Simulator {
	return &Simulator{
		Config:      cfg,
		SimConfig:   simCfg,
		SpellEngine: spells.NewEngine(cfg, seed, simCfg.IsBoss),
	}
}
...
func (s *Simulator) Run(char *character.Character) *SimulationResult {
	...
	for i := 0; i < s.SimConfig.Iterations; i++ {
		iterResult := s.runSingleIteration(char)
...
func (s *Simulator) runSingleIteration(originalChar *character.Character) *SimulationResult {
	...
	for char.CurrentTime < s.SimConfig.Duration {
		...
		if !char.Immolate.Active || immolateTimeLeft < 3*time.Second {
			if !hasImmolate || immolateTimeLeft < 3*time.Second {
				if s.tryCast(char, spells.SpellImmolate, result) {
					hasImmolate = true
					continue
				}
			}
		...
		if char.IsCooldownReady(&char.Conflagrate) {
			if s.tryCast(char, spells.SpellConflagrate, result) {
				continue
			}
		}
		...
		if char.IsCooldownReady(&char.ChaosBolt) {
			if s.tryCast(char, spells.SpellChaosBolt, result) {
				continue
			}
		}
		...
		if char.Resources.CurrentMana < manaThreshold {
			if s.tryCast(char, spells.SpellLifeTap, result) {
				continue
			}
		}
		
		// Priority 5: Incinerate (filler)
		if s.tryCast(char, spells.SpellIncinerate, result) {
			continue
		}
...
func (s *Simulator) tryCast(char *character.Character, spell spells.SpellType, result *SimulationResult) bool {
